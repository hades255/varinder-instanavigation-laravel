...
class HighlightCtr extends Controller
{
    function index(Request $request)
    {
        ...
        try {
            // Make a request to get the highlights data
            $response = $fb->get("/{$request->highlightId}/highlights", $access_token);
            $highlights = $response->getDecodedBody()['data'];
            ...
            foreach ($highlights as $highlight) {
                $highlightData[] = [
                    'type' => $highlight['form'],
                    'createdTime' => $highlight['created_time'],
                    'thumbnailURL' => $highlight['thumbnail_url'],
                ];
            }
    
            return response()->json($highlightData, 200);
